...
from .client import get_client_manager, get_client
from .tools import read_tools, write_tools, all_tools
from .utils import load_tool_prompts, get_tool_description
...
class RundeckMCPServer:
    ...
    def __init__(self, enable_write_tools: bool = False):
        ...
        self.server.update_capabilities(
            ServerCapabilities(
    ...
    async def _list_tools(self, request: ListToolsRequest) -> List[Tool]:
        ...
        if self.enable_write_tools:
            for tool_func in write_tools:
                ...
                destructive_tools = [
                    'abort_execution', 'delete_execution', 'disable_job',
                    'disable_job_schedule', 'set_execution_mode'
                ]
