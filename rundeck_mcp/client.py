    ...
    def _make_request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            raise requests.exceptions.Timeout(
                f"Request to Rundeck server '{self.name}' timed out. "
            ...
            if e.response and e.response.status_code == 401:
                raise requests.exceptions.HTTPError(
                    f"Authentication failed for server '{self.name}'. "
                    f"Please check your API token."
                ) from e
            elif e.response and e.response.status_code == 403:
                raise requests.exceptions.HTTPError(
                    f"Access denied for server '{self.name}'. "
                    f"Please check your permissions."
                ) from e
            elif e.response and e.response.status_code == 404:
                raise requests.exceptions.HTTPError(
                    f"Resource not found on server '{self.name}'. "
                    f"Please check the endpoint or resource ID."
                ) from e
            else:
                raise requests.exceptions.HTTPError(
                    f"HTTP error {e.response.status_code if e.response else 'unknown'} "
                    f"for server '{self.name}': {e}"
                ) from e
...
class ClientManager:
    ...
    def __init__(self):
        ...
        self._primary_client: Optional[RundeckClient] = None
        self._load_clients()
    ...
    def get_client(self, server_name: Optional[str] = None) -> RundeckClient:
        ...
        if not self._clients:
            raise ValueError("No Rundeck clients configured. Please set RUNDECK_URL and RUNDECK_API_TOKEN environment variables.")
...
def get_client(server_name: Optional[str] = None) -> RundeckClient:
