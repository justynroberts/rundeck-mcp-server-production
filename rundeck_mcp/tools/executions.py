...
from ..models.rundeck import JobExecution, ExecutionStatus


def get_execution_status(execution_id: str, server: Optional[str] = None) -> ExecutionStatus:
    ...
    return ExecutionStatus(
        ...
        start_time=datetime.fromisoformat(response['date-started'].replace('Z', '+00:00')) if response.get('date-started') else None,
        end_time=datetime.fromisoformat(response['date-ended'].replace('Z', '+00:00')) if response.get('date-ended') else None,
        duration=response.get('duration')
    )


def get_execution_output(execution_id: str, server: Optional[str] = None) -> Dict[str, Any]:
...
def get_executions(project: str, job_id: Optional[str] = None, 
                  status: Optional[str] = None, limit: int = 20,
                  server: Optional[str] = None) -> ListResponseModel[JobExecution]:
    ...
    for exec_data in response.get('executions', []):
        executions.append(JobExecution(
            ...
            start_time=datetime.fromisoformat(exec_data['date-started'].replace('Z', '+00:00')) if exec_data.get('date-started') else None,
            end_time=datetime.fromisoformat(exec_data['date-ended'].replace('Z', '+00:00')) if exec_data.get('date-ended') else None,
            duration=exec_data.get('duration'),
            user=exec_data.get('user')
        ))
    
    return ListResponseModel[JobExecution](
        response=executions,
        total_count=response.get('total', len(executions))
    )


def get_bulk_execution_status(execution_ids: List[str], server: Optional[str] = None) -> List[ExecutionStatus]:
...
def abort_execution(execution_id: str, server: Optional[str] = None) -> Dict[str, Any]:
...
def retry_execution(execution_id: str, options: Optional[Dict[str, Any]] = None,
                   node_filter: Optional[str] = None, server: Optional[str] = None) -> Dict[str, Any]:
...
def delete_execution(execution_id: str, server: Optional[str] = None) -> Dict[str, Any]:
