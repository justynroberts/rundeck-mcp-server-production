...
def get_execution_metrics(project: str, days: int = 30, server: Optional[str] = None) -> ExecutionMetrics:
...
def calculate_job_roi(job_id: str, manual_time_hours: float, frequency_per_month: int,
                     hourly_rate: float = 50.0, server: Optional[str] = None) -> ROIAnalysis:
...
def get_all_executions(project: str, limit: int = 100, server: Optional[str] = None) -> List[Dict[str, Any]]:
